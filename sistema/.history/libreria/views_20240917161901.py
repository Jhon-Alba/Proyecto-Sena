...
def crear_compra(request):
    ...
    breadcrumbs = [
        {'name': 'Inicio', 'url': '/dashboard'},
        {'name': 'Compras', 'url': '/libreria/compra/crear'},
    ]
    return render(request, 'compra/crear.html', {'formulario': formulario,  'breadcrumbs': breadcrumbs})

@never_cache
@login_required
def editar_compra(request, id):
    compra_ed = compra.objects.get(id=id)
    if request.method == 'POST':
