...
@login_required
def ventas(request):
    ventas = venta.objects.all()  # Cambiado a 'Venta' para reflejar la lógica de ventas
    breadcrumbs = [
        {'name': 'Inicio', 'url': '/dashboard'},
        {'name': 'Ventas', 'url': '/libreria/venta/'},  # Cambiado a 'venta'
    ]
...
def crear_venta(request):
    productos = Producto.objects.filter(activado=True)
    form = VentaForm(request.POST or None)

    if request.method == 'POST':
        if form.is_valid():
            venta = form.save()  # Guarda la venta antes de actualizar el stock
            # Supongamos que tu formulario tiene un campo llamado 'productos' con una lista de productos
            for producto in form.cleaned_data.get('productos'):
                cantidad = form.cleaned_data.get('cantidad')  # Asegúrate de que 'cantidad' esté en tu formulario
                producto.stock -= cantidad  # Disminuye el stock del producto
    ...
    return render(request, 'venta/crear.html', {'formulario': form, 'productos': productos, 'breadcrumbs': breadcrumbs})


@never_cache
@login_required
def editar_venta(request, id):
    venta_ed = get_object_or_404(venta, id=id)  # Cambiado a 'Venta'
    if request.method == 'POST':
        formulario = VentaForm(request.POST, instance=venta_ed)  # Cambiado a 'VentaForm'
        if formulario.is_valid():
            formulario.save()
            messages.success(request, '¡Venta editada con éxito!')
            return redirect('venta')  # Cambiado a 'venta'
    else:
        formulario = VentaForm(instance=venta_ed)  # Cambiado a 'VentaForm'

    breadcrumbs = [
        {'name': 'Inicio', 'url': '/dashboard'},
        {'name': 'Ventas', 'url': '/libreria/venta/'},  # Cambiado a 'venta'
        {'name': 'Editar Venta', 'url': f'/libreria/venta/editar/{id}/'},  # Cambiado a 'venta'
    ]
...
def eliminar_venta(request, id):
    ventae = get_object_or_404(venta, id=id)  # Cambiado a 'Venta'
    ventae.delete()
...
def reporte_compras_excel(request):
    ...
    headers = ["ID", "Nombre del producto", "Fecha ingreso", "Cantidad", "SubTotal", "Valor total", "Estado"]
    ws.append(headers)
...
def reporte_compras_pdf(request):
    ...
    if start_date and end_date:
        ...
        compras = compra.objects.filter(fechaingreso__range=[start_date, end_date])
    else:
        compras = compra.objects.all()

    column_widths = [30, 150, 80, 60, 80, 90, 60]
    headers = ["ID", "Nombre del producto", "Fecha ingreso", "Cantidad", "SubTotal", "Valor Total", "Estado"]
    data = [headers]
...
def reporte_productos_excel(request):
    ...
    headers = ["Id", "Nombre", "Precio", "Stock", "Categoría", "Marca", "Presentación", "Unidad Medida", "Proveedor", "Estado"]
    ws.append(headers)
    ...
    column_widths = [10, 30, 15, 20, 20, 20, 30, 30, 30, 15]
    for i, width in enumerate(column_widths, start=1):
        ws.column_dimensions[openpyxl.utils.get_column_letter(i)].width = width
...
def reporte_productos_pdf(request):
    ...
    column_widths = [25, 80, 55, 30, 60, 60, 63, 65, 110 , 50]
    headers = ["Id", "Nombre", "Precio", "Stock" , "Categoría", "Marca", "Presentación", "Unidad Medida", "Proveedor", "Estado"]
    data = [headers]
    ...
    # Ancho total de la tabla basado en la suma de los anchos de las columnas
    table_width = sum(column_widths)
    
    # Asegúrate de calcular el margen para centrar la tabla horizontalmente
    table_x = (width - table_width) / 2
    table_y = y_position - len(data) * row_height
...
def reporte_ventas_pdf(request):
    ...
    column_widths = [30, 110, 80, 60, 80, 80, 60]
    headers = ["ID", "Nombre del producto", "Fecha venta", "Cantidad", "Valor unitario", "Valor total", "Estado"]
    data = [headers]

    # Filtrar las ventas por fecha si se han proporcionado
    if start_date and end_date:
        ventas_list = venta.objects.filter(fechaventa__range=[start_date, end_date])
    else:
...
def compras(request):
    ...
    return render(request, 'compra/index.html', {'compras': compras, 'breadcrumbs': breadcrumbs})
...
def crear_compra(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            compra = form.save()  # Guarda la compra antes de actualizar el stock
            # Supongamos que tu formulario tiene un campo llamado 'productos' con una lista de productos
            for producto in form.cleaned_data.get('productos'):
                cantidad = form.cleaned_data.get('cantidad')  # Asegúrate de que 'cantidad' esté en tu formulario
                producto.stock += cantidad  # Aumenta el stock del producto
                producto.save()  # Guarda el producto con el nuevo stock
    ...
    breadcrumbs = [
        {'name': 'Inicio', 'url': '/dashboard'},
        {'name': 'Compras', 'url': '/libreria/compra/'},
        {'name': 'Crear Compra', 'url': '/libreria/compra/crear/'},
    ]

    return render(request, 'compra/crear.html', {'formulario': form, 'productos': productos, 'breadcrumbs': breadcrumbs})
